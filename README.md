...
## 开发者须知：`.bat` 必须是纯 ASCII

`setup.bat` / `run.bat` / `dist_template/启动.bat` **只允许出现 ASCII 字符**（不允许中文），换行为 CRLF。**不要加 `chcp 65001`。**

这不是风格偏好，是踩过的坑 —— `.bat` 里一旦有中文，编码就成了雷区，三种存法都会坏：

| 存法 | 后果 |
| :--- | :--- |
| UTF-8 无 BOM | 中文 Windows 的 cmd.exe 按 ANSI(936) 读取 → 中文全乱码，中文路径 `cd` 直接失败 |
| 加 `chcp 65001` | cmd 在读取途中切换代码页会**解析错位**，脚本只执行一部分 |
| LF 换行 | cmd 解析同样错位（实测报错 `'cho' 不是内部或外部命令`），**exe 根本不会被调用** |
| GBK 存本地 | 本机双击正常，但 push 到 GitHub 会被**转码成 UTF-8**，别人 clone 下来又退回上面第一条 |

纯 ASCII 的字节在任何代码页下含义都一样，没有任何转换能弄坏它。所以启动脚本刻意**不含中文**，也**不写死中文目录名** —— 改为按 `_internal` 标记自动定位 exe。

中文提示改由程序自己打印（`bill_ui.py` 里的 `BANNER`）：Python 在 Windows 控制台走 `WriteConsoleW`，中文显示与代码页无关。

改了 `.bat` 之后一定要实机跑一遍验证：`cmd /c 启动.bat`，确认窗口里出现中文横幅、服务起来、浏览器自动打开。
